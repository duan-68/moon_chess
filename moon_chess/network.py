...
async def _poll_event(event: threading.Event, interval: float = 0.1) -> None:
    """异步轮询 threading.Event，set 后返回。"""
...
async def _client_loop(session: NetworkSession, url: str, create: bool) -> None:
    ...
    try:
        async with ws:
            ...
            # 等待匹配完成（带超时），仅当房间码/匹配未完成时等待
            try:
                await asyncio.wait_for(
                    _poll_event(session._started), timeout=MATCH_TIMEOUT
                )
            except asyncio.TimeoutError:
                ...
                pass  # _poll_event 在 event set 时正常返回
