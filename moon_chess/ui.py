...
from moon_chess.player import Player
from moon_chess.network import get_relay_url, set_relay_url
...
if TYPE_CHECKING:
    from moon_chess.network import NetworkSession
...
def _make_fonts() -> dict[str, pygame.font.Font]:
    """创建字体缓存，优先使用系统中文字体。"""
    chinese_names = [
        "microsoft yahei", "simhei", "noto sans cjk",
        "arial unicode ms", "fangsong",
    ]
    cn_name = None
    ...
    if cn_name:
        return {
            "small": pygame.font.SysFont(cn_name, 18),
            "mid": pygame.font.SysFont(cn_name, 24),
            "big": pygame.font.SysFont(cn_name, 36),
            "title": pygame.font.SysFont(cn_name, 48),
            "huge": pygame.font.SysFont(cn_name, 60),
        }
    return {
...
class MoonChessUI:
    ...
    def show_online_lobby(self, net: NetworkSession) -> bool:
        ...
        while self._running and choice is None:
            ...
            # ── 服务器设置区域 ──────────────────────────
            _text_center(self.screen, "中继服务器 (点击修改)", "small", TEXT_DIM, 525)
            if editing_server:
                c = P1_COLOR
    ...
    def _host_room(self, net: NetworkSession) -> bool:
        ...
        while self._running:
            ...
            if net.error: return False
            if net.room_code and net.started: return True
            self.screen.fill(BG)
            _text_center(self.screen, "等待对手加入...", "title", TEXT_MAIN, 200)
            if net.room_code:
                _text_center(self.screen, f"房间码: {net.room_code}", "big", P1_COLOR, 300)
                _text_center(self.screen, "告诉对手这个 4 位码", "small", TEXT_DIM, 360)
            else:
                _text_center(self.screen, "正在连接中继服务器...", "small", TEXT_DIM, 300)
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_ESCAPE:
                        ...
                        return False
                    if event.key == pygame.K_RETURN and len(code) == 4 and not joining:
                        net.join_room(code); joining = True; status = "加入中..."
                    elif event.key == pygame.K_BACKSPACE and not joining: code = code[:-1]
                    elif not joining and len(code) < 4 and event.unicode.isalpha():
                        code += event.unicode.upper()
            if joining and net.error: status = net.error; joining = False
            if joining and net.started: return True
            self.screen.fill(BG)
            _text_center(self.screen, "加入房间", "title", TEXT_MAIN, 150)
            _text_center(self.screen, "输入对手的 4 位房间码", "small", TEXT_DIM, 220)
            box_w, box_h = 200, 56; box_x = WINDOW_W // 2 - box_w // 2; box_y = 300
            c = GRID_LINE if not joining else CELL_BG
            pygame.draw.rect(self.screen, c, (box_x, box_y, box_w, box_h), border_radius=12)
            display = code.ljust(4, "_") if not joining else code
            txt = _get_fonts()["title"].render(display, True, TEXT_MAIN if code else TEXT_DIM)
            self.screen.blit(txt, txt.get_rect(center=(WINDOW_W // 2, box_y + box_h // 2)))
            if status:
                sc = CELL_HOVER if "加入" in status else OLDEST_RING
                _text_center(self.screen, status, "small", sc, 400)
            _text_center(self.screen, "回车加入  ESC 返回", "small", TEXT_DIM, 500)
            pygame.display.flip(); self.clock.tick(FPS)
        return False
